...
document.addEventListener('DOMContentLoaded', () => {
  try { initTelegram(); } catch (_) {}
  try { currentLang = window.YD_INITIAL_LANGUAGE || localStorage.getItem('lang') || detectLanguage(); } catch (_) { currentLang = 'en'; }
  try { localStorage.setItem('lang', currentLang); } catch (_) {}
  try { initParticles(); } catch (_) {}
  try { initNavigation(); } catch (_) {}
  try { loadSettings(); } catch (_) {}
  try { renderThemeColors(); } catch (_) {}
  try { updateLanguageUI(); } catch (_) {}
  // Splash timing is owned exclusively by boot.js. Never block startup here.
});
